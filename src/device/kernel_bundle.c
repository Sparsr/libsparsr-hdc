 ...
 *
 * The loop needs a WMEM row that varies at run time, which is what the register-indirect
 ...
 *
 * The accumulator lives in a WMEM row rather than staying in a wide register between
 * batches, so a bundle longer than WMEM holds can run as several batches and pick up where
 ...
 * how many rows to fill. A count larger than WMEM would read rows the host never wrote,
